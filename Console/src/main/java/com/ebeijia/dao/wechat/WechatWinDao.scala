...
@Repository("WechatWinningDao")
class WechatWinDao extends BaseDaoImplHibernate[TblWechatWinning] with BaseDao[TblWechatWinning] {
  def getWechatConfigList: List[TblWechatWinning] = {
    val hql: String = "FROM TblWechatWinning "
   ...
   */
  def countTotalNum(wechatConfig: TblWechatWinning): Integer = {
    val hql: String = "SELECT COUNT(*) FROM TblWechatWinning"
