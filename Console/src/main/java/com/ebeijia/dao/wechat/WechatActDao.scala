...
class WechatActDao extends BaseDaoImplHibernate[TblWechatAct] with BaseDao[TblWechatAct] {
  def getWechatConfigList: List[TblWechatAct] = {
    val hql: String = "FROM TblWechatAct "
   ...
   */
  def countTotalNum(wechatConfig: TblWechatAct): Integer = {
    val hql: String = "SELECT COUNT(*) FROM TblWechatAct"
