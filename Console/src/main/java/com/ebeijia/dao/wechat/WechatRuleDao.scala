...
@Repository("WechatRuleDao")
class WechatRuleDao extends BaseDaoImplHibernate[TblWechatRule] with BaseDao[TblWechatRule] {
  def getWechatConfigList: List[TblWechatRule] = {
    val hql: String = "FROM TblWechatRule "
   ...
   */
  def countTotalNum(wechatConfig: TblWechatRule): Integer = {
    val hql: String = "SELECT COUNT(*) FROM TblWechatRule"
