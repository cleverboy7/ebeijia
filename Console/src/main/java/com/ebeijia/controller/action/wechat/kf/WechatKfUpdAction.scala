...
class WechatKfUpdAction {
  private val logger: Logger = LoggerFactory.getLogger(classOf[WechatKfUpdAction])
  @Autowired
  ...
  @ResponseBody def upd(session: HttpSession, request: HttpServletRequest): Map[String, AnyRef] = {
    ...
    try {
      val kfId: TblWechatKfId = new TblWechatKfId
      kfId.setKfId(id)
      kfId.setMchtId(mchtId)
      val tblWechatKf: TblWechatKf = wechatKfService.getById(kfId)
      if (tblWechatKf == null) {
        AjaxResp.getReturn(Constant.ERROR_CODE, "客服账号不存在", "")
      }
      val respMsg: String = wechatKfService.upd(mchtId, kf)
      if (respMsg == null) {
      ...
      case e: Exception => {
        logger.info("修改多客服失败")
        AjaxResp.getReturn(Constant.ERROR_CODE, "修改多客服失败,请联系管理员或稍后再试", "")
      }
    }
  }
  ...
  @ResponseBody def del(session: HttpSession, request: HttpServletRequest): Map[String, AnyRef] = {
    ...
    try {
      ...
      val respMsg: String = wechatKfService.del(mchtId, id)
      if (respMsg == null) {
        AjaxResp.getReturn(Constant.SUCCESS_CODE, "", "")
      }
      else {
      ...
      case e: Exception => {
        logger.info("删除多客服失败")
        AjaxResp.getReturn(Constant.ERROR_CODE, "删除多客服失败,请联系管理员或稍后再试", "")
      }
    }
  }
}
