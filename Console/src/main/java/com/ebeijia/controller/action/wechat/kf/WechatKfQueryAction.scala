...
class WechatKfQueryAction {
  @Autowired
  private val wechatKfService: WechatKfService = null
  private val logger: Logger = LoggerFactory.getLogger(classOf[WechatKfQueryAction])
  ...
  @ResponseBody def query(session: HttpSession, request: HttpServletRequest): Map[String, AnyRef] = {
    ...
    try {
      val mapTmp: Map[String, AnyRef] = wechatKfService.findBySql(mchtId, pageData)
      map.put("kfList", mapTmp.get("list"))
      ...
      case e: Exception => {
        logger.info("微信客服查询失败")
        AjaxResp.getReturn(Constant.ERROR_CODE, "微信客服查询失败,请联系管理员或稍后再试", "")
      }
    }
  }
}
